...
/*******************************************************************************
* Function Name: Thermistor_Calc_GetResistance
********************************************************************************
*
* Summary:
                                      ...
                                      
{
    ...
    resT = Thermistor_Calc_REF_RESISTOR * ((int32)vThermistor);
    if (vReference < 0)
    {
        temp = -vReference;
    ...
    return ((uint32)((uint32)resT << Thermistor_Calc_REF_RES_SHIFT));
}


/*******************************************************************************
* Function Name: Thermistor_Calc_GetTemperature
********************************************************************************
*
* Summary:
...
int16 Thermistor_Calc_GetTemperature(uint32 resT) 
{
    int16 tempTR;
        ...
        #if(!CY_PSOC3)
            stEqn = (float32)(Thermistor_Calc_THA + (Thermistor_Calc_THB * logrT) + 
                             (Thermistor_Calc_THC * pow((float64)logrT, (float32)3)));
        #else
            stEqn = (float32)(Thermistor_Calc_THA + (Thermistor_Calc_THB * logrT) + 
                             (Thermistor_Calc_THC * pow(logrT, (float32)3)));
        #endif  /* (!CY_PSOC3) */

        tempTR = (int16)((float32)((((1.0 / stEqn) - Thermistor_Calc_K2C) * (float32)Thermistor_Calc_SCALE) + 0.5));

    #else /* Thermistor_Calc_IMPLEMENTATION == Thermistor_Calc_LUT_METHOD */

        uint16 mid;
        uint16 first = 0u;
        uint16 last = Thermistor_Calc_LUT_SIZE - 1u;
        ...
        while (first < last)
        {
            ...
            if ((0u == mid) || ((Thermistor_Calc_LUT_SIZE - 1u) == mid) || (resT == Thermistor_Calc_LUT[mid]))
            {
                last = mid;
                break;
            }
            else if (Thermistor_Calc_LUT[mid] > resT)
            {
                first = mid + 1u;
            }
            else /* (Thermistor_Calc_LUT[mid] < resT) */
            {
                last = mid - 1u;
            }
        }
        ...
        if ((Thermistor_Calc_LUT[last] > resT) && (last < (Thermistor_Calc_LUT_SIZE - 1u)) &&
           ((Thermistor_Calc_LUT[last] - resT) > (resT - Thermistor_Calc_LUT[last + 1u])))
        {
            last++;
        }
        else if ((Thermistor_Calc_LUT[last] < resT) && (last > 0u) &&
                ((Thermistor_Calc_LUT[last - 1u] - resT) < (resT - Thermistor_Calc_LUT[last])))
        {
            last--;
        }
        else
        ...
        tempTR = Thermistor_Calc_MIN_TEMP + (int16)((uint16)(last * Thermistor_Calc_ACCURACY));
